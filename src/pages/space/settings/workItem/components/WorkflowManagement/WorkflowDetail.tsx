...
import { ProcessViewWithEditMode } from '@/components/ProcessView';
import { BasicMap, TestMap } from '@/components/ProcessView/exampleMap';
import { addEdge } from '@/components/ProcessView/utils';
import type { ProcessNodeType, ProcessNodeIdType } from '@/components/ProcessView/types';

interface WorkflowDetailProps {
  workflow: WorkflowType;
  onBack: () => void;
  onUpdate: (id: number, values: { name: string }) => Promise<void>;
...
const WorkflowDetail: React.FC<WorkflowDetailProps> = ({
  ...
  const [processNodes, setProcessNodes] = useState<ProcessNodeType[]>(Object.values(BasicMap));
  ...
  const handleSave = async () => {
    try {
      ...
      await onUpdate(workflow.id, values);
  ...
  const handleEdgeCreate = (sourceNodeId: ProcessNodeIdType, targetNodeId: ProcessNodeIdType) => {
    try {
      // 调用 addEdge 方法创建新边
      const updatedNodes = addEdge(processNodes, sourceNodeId, targetNodeId);
      // 更新状态，重新渲染流程图
      setProcessNodes(updatedNodes);
      message.success('边创建成功');
    } catch (error: any) {
      console.error('Failed to create edge:', error);
      message.error(error?.message || '边创建失败');
    }
  };

  const handleAddEdge = (sourceNodeId: ProcessNodeIdType, targetNodeId: ProcessNodeIdType, newNodes: ProcessNodeType[]) => {
            ...
            <div className="w-[360px] bg-white border-l border-[#f0f0f0] flex flex-col">
              ...
              <div className="px-4 h-12 flex-shrink-0 border-b border-[#f0f0f0] flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-[#262626]">开始</span>
                </div>
                <Button
                  type="text"
                  size="small"
                ...
                {sidebarTab === 'info' && (
                  <div>
                    ...
                    <Form layout="vertical">
                      ...
                      >
                        <div className="text-sm text-[#262626] font-mono bg-[#f5f5f5] px-2 py-1 rounded">start</div>
                      </Form.Item>
                    </Form>
                  </div>
                )}
